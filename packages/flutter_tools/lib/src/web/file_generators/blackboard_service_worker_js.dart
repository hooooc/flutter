import 'flutter_service_worker_js.dart';

String generateBlackboardServiceWorker(
...
self.addEventListener("fetch", (event) => {
  ...
  if (event.request.url.startsWith(origin)) {
    key = event.request.url.substring(origin.length + 1);
  } else if (assetBase && event.request.url.startsWith(assetBase)) {
    key = event.request.url.substring(assetBase.length);
  }
  // chenshitao end
